...
export default function DirectBookingPage() {
  ...
  if (err) {
    return (
      <div className="py-20 text-center">
        ...
        <Link to="/browse" className="text-[#984A23] mt-3 inline-block">Browse practitioners →</Link>
      </div>
    );
  }
  if (!p) return <div className="py-20 text-center text-[#5C4E43]">Loading…</div>;
  ...
  return (
    <div className="mx-auto max-w-3xl px-5 md:px-10 py-10">
      ...
      <div className="text-sm text-[#5C4E43] mt-2 flex items-center gap-3">
      ...
      <p className="mt-4 text-[#5C4E43]">{p.bio}</p>
      ...
      <div className="space-y-2">
        {p.services.map((s) => (
          <button
            ...
            className="w-full text-left bg-white rounded-2xl border border-[#E2D9CF] hover:border-[#984A23] p-4 flex items-center justify-between"
          >
            <div>
              <div className="text-xs text-[#984A23] font-semibold uppercase tracking-widest">{s.category_name}</div>
              <div className="font-semibold">{s.name}</div>
              <div className="text-xs text-[#5C4E43]">{Math.round(s.duration_minutes_min / 60 * 10) / 10}h</div>
            </div>
            <div className="text-right">
              <div className="font-serif text-lg">${s.price_min}</div>
              <div className="text-xs text-[#5C4E43]">${(s.price_min * 0.25).toFixed(2)} deposit</div>
            </div>
          </button>
        ))}
      </div>
