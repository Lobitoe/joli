    ...
    <div className="mx-auto max-w-7xl px-5 md:px-10 py-10">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <div className="md:col-span-1">
          <div className="aspect-square rounded-3xl overflow-hidden border border-[#E2D9CF]">
            <img src={p.profile_photo_url || p.portfolio[0]?.image_url} alt={p.display_name} className="w-full h-full object-cover" />
          </div>
        </div>
        <div className="md:col-span-2 flex flex-col justify-between">
          <div>
            <div className="flex items-center gap-2 text-xs uppercase tracking-widest text-[#984A23] font-bold">
            ...
            <div className="flex items-center gap-3 text-sm text-[#5C4E43] mt-2">
              <span className="flex items-center gap-1"><Star size={14} className="fill-[#E1A100] text-[#E1A100]" /> {p.avg_rating || "New"} ({p.total_reviews})</span>
              <span>·</span>
            ...
            <div className="mt-4 flex flex-wrap items-center gap-3 text-sm">
              {p.instagram_handle && (
                <a href={`https://instagram.com/${p.instagram_handle.replace("@", "")}`} target="_blank" rel="noreferrer" className="flex items-center gap-1 hover:text-[#984A23]">
                  <Instagram size={16} /> {p.instagram_handle}
                </a>
              )}
              {p.whatsapp_number && (
                <span className="flex items-center gap-1 text-[#5C4E43]"><MessageCircle size={16} /> {p.whatsapp_number}</span>
              )}
              {p.service_neighbourhoods?.length > 0 && (
                <span className="text-[#5C4E43]">Serves: {p.service_neighbourhoods.join(", ")}</span>
              )}
              {p.service_radius_km && (
                <span className="text-[#5C4E43]">Travels up to {p.service_radius_km} km</span>
              )}
            </div>
          ...
          <div className="mt-6 flex gap-3 flex-wrap">
            <button
              ...
              className={`inline-flex items-center gap-2 rounded-full px-5 py-2.5 border ${favorite ? "bg-[#984A23] text-white border-[#984A23]" : "border-[#E2D9CF] hover:border-[#984A23]"}`}
            >
            ...
            <button
              ...
              className="inline-flex items-center gap-2 rounded-full px-5 py-2.5 border border-[#E2D9CF] hover:border-[#984A23]"
            >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
            {filteredPortfolio.map((item) => (
              <div key={item.id} data-testid={PROFILE.portfolioImage} className="relative aspect-[4/5] rounded-2xl overflow-hidden border border-[#E2D9CF]">
          ...
          <div className="mt-10">
            ...
            {p.reviews.length === 0 ? (
              <div className="text-[#5C4E43] bg-[#F3EFEA] p-4 rounded-2xl">No reviews yet.</div>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {p.reviews.map((r) => (
                  <div key={r.id} className="bg-white rounded-2xl border border-[#E2D9CF] p-4">
                    <div className="flex items-center justify-between">
                      <div className="font-semibold">{r.client_name}</div>
                      <div className="flex items-center gap-1 text-sm">
                        <Star size={14} className="fill-[#E1A100] text-[#E1A100]" /> {r.rating}
                      </div>
                    </div>
                    <p className="text-[#5C4E43] mt-2 text-sm leading-relaxed">{r.text}</p>
                  </div>
                ))}
              </div>
            )}
        ...
        <div className="lg:col-span-1">
          <div className="sticky top-20 bg-white rounded-2xl border border-[#E2D9CF] p-5">
            ...
            <p className="text-xs text-[#5C4E43] mb-4">
              Cancellation: <span className="text-[#2B231D]">{p.cancellation_policy}</span>
            </p>
            <div className="space-y-5 max-h-[600px] overflow-y-auto pr-1">
              {Object.entries(servicesByCategory).map(([cat, svcs]) => (
                <div key={cat}>
                  <div className="overline text-[10px] font-bold tracking-[0.2em] uppercase text-[#984A23] mb-2">{cat}</div>
                  <div className="space-y-2">
                    {svcs.map((s) => (
                      <div key={s.id} data-testid={PROFILE.serviceItem} className="rounded-xl border border-[#E2D9CF] p-3 hover:border-[#984A23] transition-colors">
                        <div className="flex items-start justify-between gap-2">
                          <div>
                            <div className="font-semibold">{s.name}</div>
                            <div className="text-xs text-[#5C4E43] flex items-center gap-2 mt-1">
                        ...
                        <Link
                          ...
                          className="mt-3 inline-flex items-center justify-center w-full rounded-full bg-[#984A23] hover:bg-[#7e3d1d] text-white text-sm font-semibold py-2 transition-colors"
                        >
            ...
            <div className="mt-4 text-xs text-[#5C4E43] flex items-start gap-2">
              <CheckCircle2 size={14} className="text-[#4A5D23] flex-shrink-0 mt-0.5" />
