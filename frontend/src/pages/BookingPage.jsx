...
export default function BookingPage() {
  ...
  if (!practitioner || !service) {
    return <div className="py-20 text-center text-[#5C4E43]">Loading…</div>;
  }

  if (confirmation) {
    return (
      <div className="mx-auto max-w-2xl px-5 md:px-10 py-16">
        <div className="bg-white rounded-3xl border border-[#E2D9CF] p-8 text-center fade-in" data-testid={BOOKING.confirmationStatus}>
          <div className="inline-flex h-16 w-16 items-center justify-center rounded-full bg-[#4A5D23] text-white mb-4">
          ...
          <div className="mt-6 bg-[#F3EFEA] rounded-2xl p-5 text-left text-sm space-y-2">
            <div><span className="text-[#5C4E43]">Service:</span> <span className="font-semibold">{service.name}</span></div>
            <div><span className="text-[#5C4E43]">When:</span> <span className="font-semibold">{confirmation.booking_date} at {confirmation.start_time} – {confirmation.end_time}</span></div>
            <div><span className="text-[#5C4E43]">Deposit paid:</span> <span className="font-semibold">${confirmation.deposit_amount}</span></div>
            <div><span className="text-[#5C4E43]">Remaining (at appointment):</span> <span className="font-semibold">${(confirmation.quoted_price - confirmation.deposit_amount).toFixed(2)}</span></div>
            <div><span className="text-[#5C4E43]">Pay with:</span> <span className="font-semibold">{practitioner.accepted_payments.join(" / ")}</span></div>
          </div>
          <div className="mt-6 flex gap-3 justify-center">
            <button onClick={() => nav("/dashboard/client")} className="rounded-full bg-[#984A23] text-white px-6 py-3 font-semibold hover:bg-[#7e3d1d]">
              View my bookings
            </button>
            <button onClick={() => nav("/browse")} className="rounded-full border border-[#E2D9CF] px-6 py-3 font-semibold hover:border-[#984A23]">
              Keep browsing
            </button>
          </div>
        </div>
  ...
  return (
    <div className="mx-auto max-w-5xl px-5 md:px-10 py-10">
      <button onClick={() => nav(-1)} className="inline-flex items-center gap-1 text-sm text-[#5C4E43] hover:text-[#984A23] mb-4">
        <ArrowLeft size={14} /> Back
      </button>
      <h1 className="font-serif text-4xl md:text-5xl">Book your appointment</h1>
      <p className="text-[#5C4E43] mt-2">with {practitioner.display_name}</p>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mt-8">
        <div className="lg:col-span-2 space-y-6">
          {/* Date */}
          <div className="bg-white rounded-2xl border border-[#E2D9CF] p-5">
            ...
            <input
              ...
              className="h-12 rounded-full border border-[#E2D9CF] px-5 bg-[#F3EFEA] outline-none focus:ring-2 focus:ring-[#984A23]"
            />
          </div>
          ...
          <div className="bg-white rounded-2xl border border-[#E2D9CF] p-5">
            ...
            {slots.length === 0 ? (
              <div className="text-sm text-[#5C4E43]">No slots available on this date. Try another day.</div>
            ) : (
              <div className="grid grid-cols-3 md:grid-cols-5 gap-2">
                {slots.map((s) => (
                  <button
                    ...
                    onClick={() => setSlot(s)}
                    className={`h-11 rounded-full border text-sm font-medium transition-colors ${slot === s ? "bg-[#984A23] text-white border-[#984A23]" : "border-[#E2D9CF] hover:border-[#984A23]"}`}
                  >
                    {s}
                  </button>
                ))}
          ...
          <div className="bg-white rounded-2xl border border-[#E2D9CF] p-5">
            ...
            <textarea
              ...
              className="w-full rounded-2xl border border-[#E2D9CF] p-4 bg-[#F3EFEA] outline-none focus:ring-2 focus:ring-[#984A23]"
            />
          </div>
        </div>
        ...
        <div className="lg:col-span-1">
          <div className="sticky top-20 bg-white rounded-2xl border border-[#E2D9CF] p-5">
            <div className="font-serif text-2xl mb-4">Booking summary</div>
            <div className="space-y-3 text-sm">
              <div>
                <div className="text-[#5C4E43] text-xs">Service</div>
                <div className="font-semibold">{service.name}</div>
              </div>
              <div>
                <div className="text-[#5C4E43] text-xs">Duration</div>
                <div>{Math.round(service.duration_minutes_min / 60 * 10) / 10} hours</div>
              </div>
              <div>
                <div className="text-[#5C4E43] text-xs">Date & time</div>
                <div>{date}{slot ? ` · ${slot}` : ""}</div>
              </div>
              <hr className="border-[#E2D9CF]" />
              <div className="flex justify-between"><span>Total</span><span className="font-semibold">${service.price_min.toFixed(2)}</span></div>
              <div className="flex justify-between text-[#984A23] font-semibold"><span>Deposit (25%)</span><span>${deposit.toFixed(2)}</span></div>
              <div className="flex justify-between text-[#5C4E43]"><span>Remaining at appointment</span><span>${remaining.toFixed(2)}</span></div>
              <div className="text-xs text-[#5C4E43] mt-2">
                Remainder via {practitioner.accepted_payments.join(" / ")}.
              </div>
              {directLink && (
                <div className="text-xs bg-[#EEDDCB] text-[#4A2B12] rounded-xl p-2">
            ...
            <button
              ...
              className="mt-5 w-full inline-flex items-center justify-center gap-2 rounded-full bg-[#984A23] hover:bg-[#7e3d1d] disabled:opacity-50 text-white font-semibold py-3 transition-colors"
            >
              <CreditCard size={16} /> {submitting ? "Processing…" : `Pay $${deposit.toFixed(2)} deposit (mocked)`}
            </button>
            <div className="text-xs text-[#5C4E43] mt-3 text-center">
