  ...
  return (
    <div className="mx-auto max-w-7xl px-5 md:px-10 py-10">
      <div className="flex items-end justify-between mb-6">
        <div>
          <div className="overline text-xs tracking-[0.2em] uppercase font-bold text-[#C8552F]">Discover</div>
      ...
      {/* Filters */}
      <div className="bg-white rounded-2xl border border-[#D9CFBE] p-4 mb-8">
        <div className="flex items-center gap-2 mb-3 text-xs uppercase tracking-widest text-[#6E5F50]">
          <Filter size={14} /> Filters
        </div>
        <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
          <div>
            <label className="block text-xs text-[#6E5F50] mb-1">City</label>
            <select
              data-testid={BROWSE.filterCity}
              value={city}
              onChange={(e) => update("city", e.target.value)}
              className="w-full h-11 rounded-full border border-[#D9CFBE] px-4 bg-[#EFE8DA] outline-none focus:ring-2 focus:ring-[#C8552F]"
            >
              {CITIES.map((c) => (
          ...
          <div>
            <label className="block text-xs text-[#6E5F50] mb-1">Style / Category</label>
            <select
          ...
          <div>
            <label className="block text-xs text-[#6E5F50] mb-1">Service mode</label>
            <select
          ...
          <div>
            <label className="block text-xs text-[#6E5F50] mb-1">Type</label>
            <select
              value={type}
      ...
      {loading ? (
        <div className="py-20 text-center text-[#6E5F50]">Loading practitioners…</div>
      ) : results.length === 0 ? (
        <div className="py-20 text-center" data-testid={BROWSE.emptyState}>
          <Sparkles className="mx-auto text-[#C8552F]" />
          <div className="font-serif text-2xl mt-3">No practitioners match these filters.</div>
          <p className="text-[#6E5F50] mt-2">Try removing a filter, or pick a different style.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {results.map((p) => (
            <Link
              ...
              className="bg-white rounded-2xl overflow-hidden border border-[#D9CFBE] hover:shadow-lg transition-shadow fade-in"
            >
              ...
              <div className="p-5">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-1 text-sm">
                    ...
                    {p.total_reviews ? <span className="text-[#6E5F50]">({p.total_reviews})</span> : null}
                  </div>
                </div>
                <div className="text-sm text-[#6E5F50] mt-1 flex items-center gap-1">
                ...
                <div className="mt-3 flex items-center justify-between text-sm">
                  <span className="text-[#C8552F] font-semibold">From ${p.starting_price ?? "—"}</span>
