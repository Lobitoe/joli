...
export default function AdminVerificationQueue() {
  ...
  return (
    <div className="mx-auto max-w-5xl px-5 md:px-10 py-10">
      <Link to="/dashboard/admin" className="inline-flex items-center gap-1 text-sm text-[#5C4E43] hover:text-[#984A23] mb-4">
        <ArrowLeft size={14} /> Admin
      </Link>
      <h1 className="font-serif text-4xl md:text-5xl">Verification queue</h1>
      <p className="text-[#5C4E43] mt-1 mb-6">{items.length} pending</p>

      {items.length === 0 ? (
        <div className="bg-white rounded-2xl border border-[#E2D9CF] p-8 text-center text-[#5C4E43]">
          Inbox zero. Nice.
        </div>
      ) : (
        <div className="space-y-3">
          {items.map((it) => {
            ...
            return (
              <div key={it.id} data-testid={ADMIN_VERIFY.queueItem} className="bg-white rounded-2xl border border-[#E2D9CF] p-5">
                <div className="flex items-start justify-between gap-4 flex-wrap">
                  <div>
                    <div className="text-xs uppercase tracking-widest text-[#984A23] font-bold">{it.verification_type.replace("_", " ")}</div>
                    <div className="font-serif text-xl mt-1">{it.practitioner_name}</div>
                    <div className="text-sm text-[#5C4E43] capitalize">{it.practitioner_type?.replace("_", " ")} · {it.practitioner_email}</div>
                    <a href={it.document_url} target="_blank" rel="noreferrer" className="mt-2 inline-flex items-center gap-1 text-sm text-[#984A23] hover:underline">
                      <ExternalLink size={12} /> View document
                    </a>
                    {candidateBadges.length > 0 && (
                      <div className="mt-3">
                        <div className="text-xs text-[#5C4E43] mb-1">Grant on approval:</div>
                  ...
                  <div className="flex gap-2">
                    <button
                      ...
                      onClick={() => approve(it)}
                      className="rounded-full bg-[#4A5D23] hover:bg-[#3a4a1c] text-white font-semibold px-4 py-2 text-sm inline-flex items-center gap-1"
                    >
                    ...
                    <button
                      ...
                      className="rounded-full border border-[#E2D9CF] hover:border-red-500 hover:text-red-500 font-semibold px-4 py-2 text-sm inline-flex items-center gap-1"
                    >
      ...
      {reasonFor && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl max-w-md w-full p-6">
            ...
            <div className="text-sm text-[#5C4E43] mb-3">Tell the practitioner why so they can re-submit.</div>
            <textarea rows={4} value={reason} onChange={(e) => setReason(e.target.value)} className="w-full rounded-2xl border border-[#E2D9CF] p-3 bg-[#F3EFEA] outline-none" placeholder="e.g. ID photo is blurry — please re-upload" />
            <div className="flex gap-2 mt-4 justify-end">
              <button onClick={() => { setReasonFor(null); setReason(""); }} className="rounded-full border border-[#E2D9CF] px-4 py-2 text-sm">Cancel</button>
