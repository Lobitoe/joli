...
export default function BlogIndexPage() {
  ...
  return (
    <div className="mx-auto max-w-7xl px-5 md:px-10 py-12">
      <div className="flex items-center gap-2 text-xs uppercase tracking-[0.2em] font-bold text-[#C8552F]">
        <Newspaper size={14} /> The Joli Journal
      </div>
      <h1 className="font-serif text-5xl md:text-6xl mt-2">Stories, guides & local discovery</h1>
      <p className="text-[#6E5F50] mt-3 max-w-2xl">
      ...
      <div className="mt-8 flex flex-wrap gap-2">
        ...
        <div className="w-px bg-[#D9CFBE] mx-1"></div>
        {CITIES.map((c) => (
          <button
            key={c}
      ...
      {loading ? (
        <div className="py-20 text-center text-[#6E5F50]">Loading…</div>
      ) : posts.length === 0 ? (
        <div className="py-20 text-center text-[#6E5F50]">No posts match those filters yet.</div>
      ) : (
        <>
          {/* Featured */}
          {featured && (
            ...
            >
              ...
              <div className="md:col-span-2">
                ...
                <h2 className="font-serif text-3xl md:text-4xl mt-3 leading-tight group-hover:text-[#C8552F] transition-colors">
                  {featured.title}
                </h2>
                <p className="text-[#6E5F50] mt-3">{featured.excerpt}</p>
                <div className="mt-4 inline-flex items-center gap-1 text-sm font-semibold text-[#C8552F]">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mt-12">
            {rest.map((p) => (
              <Link
                ...
                to={`/blog/${p.slug}`}
                data-testid={BLOG.postCard}
                className="group block bg-white border border-[#D9CFBE] rounded-2xl overflow-hidden hover:shadow-lg transition-shadow"
              >
                <div className="aspect-[16/10] overflow-hidden">
                  <img src={p.hero_image} alt={p.title} className="w-full h-full object-cover img-hover-scale" />
                </div>
                <div className="p-5">
                  <div className="text-[10px] uppercase tracking-[0.2em] font-bold text-[#C8552F]">
                    {p.category}{p.city ? ` · ${p.city}` : ""}
                  </div>
                  <div className="font-serif text-xl mt-2 leading-tight group-hover:text-[#C8552F] transition-colors">{p.title}</div>
                  <p className="text-sm text-[#6E5F50] mt-2 line-clamp-3">{p.excerpt}</p>
                </div>
              </Link>
            ))}
          </div>
