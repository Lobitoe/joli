...
export default function PractitionerOnboarding() {
  ...
  if (!loaded) return <div className="py-20 text-center text-[#5C4E43]">Loading…</div>;

  return (
    <div className="mx-auto max-w-3xl px-5 md:px-10 py-10">
      <h1 className="font-serif text-4xl md:text-5xl">Set up your studio</h1>
      <p className="text-[#5C4E43] mt-2">Tell clients who you are, where you work, and what you offer.</p>

      <form onSubmit={submit} className="mt-8 bg-white rounded-2xl border border-[#E2D9CF] p-6 space-y-5">
        <Field label="Display name (e.g., Blessing's Braids YYC)">
          <input
            ...
            className="w-full h-12 rounded-full border border-[#E2D9CF] px-5 bg-[#F3EFEA]"
          />
        </Field>
        <Field label="About you">
          <textarea
            ...
            data-testid="onb-bio"
            className="w-full rounded-2xl border border-[#E2D9CF] p-4 bg-[#F3EFEA]"
          />
        </Field>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <Field label="Practitioner type">
            <select value={form.practitioner_type} onChange={(e) => setForm({ ...form, practitioner_type: e.target.value })} className="w-full h-12 rounded-full border border-[#E2D9CF] px-5 bg-[#F3EFEA]">
              {TYPES.map(([v, l]) => <option key={v} value={v}>{l}</option>)}
            </select>
          </Field>
          <Field label="Location type">
            <select value={form.location_type} onChange={(e) => setForm({ ...form, location_type: e.target.value })} className="w-full h-12 rounded-full border border-[#E2D9CF] px-5 bg-[#F3EFEA]">
              {LOCATION_TYPES.map(([v, l]) => <option key={v} value={v}>{l}</option>)}
            </select>
          </Field>
          <Field label="Service mode">
            <select value={form.service_mode} onChange={(e) => setForm({ ...form, service_mode: e.target.value })} className="w-full h-12 rounded-full border border-[#E2D9CF] px-5 bg-[#F3EFEA]">
              {SERVICE_MODES.map(([v, l]) => <option key={v} value={v}>{l}</option>)}
            </select>
          </Field>
          <Field label="City">
            <input value={form.city} onChange={(e) => setForm({ ...form, city: e.target.value })} className="w-full h-12 rounded-full border border-[#E2D9CF] px-5 bg-[#F3EFEA]" />
          </Field>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <Field label="Instagram handle">
            <input value={form.instagram_handle} onChange={(e) => setForm({ ...form, instagram_handle: e.target.value })} placeholder="@you" className="w-full h-12 rounded-full border border-[#E2D9CF] px-5 bg-[#F3EFEA]" />
          </Field>
          <Field label="WhatsApp number">
            <input value={form.whatsapp_number} onChange={(e) => setForm({ ...form, whatsapp_number: e.target.value })} placeholder="+1-403-555-0100" className="w-full h-12 rounded-full border border-[#E2D9CF] px-5 bg-[#F3EFEA]" />
          </Field>
        </div>
        ...
        <button type="submit" disabled={saving} className="w-full rounded-full bg-[#984A23] hover:bg-[#7e3d1d] text-white font-semibold h-12">
...
function Field({ label, children }) {
  return (
    <div>
      <label className="block text-xs font-semibold uppercase tracking-widest text-[#5C4E43] mb-1">{label}</label>
      {children}
    </div>
  );
}
