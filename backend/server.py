"""Joli FastAPI server — Multicultural Beauty Marketplace MVP."""
...
@api.post("/me/practitioner/verifications")
async def submit_verification(body: VerificationIn, user: dict = Depends(get_current_user)):
    ...
    if not prac:
        raise HTTPException(status_code=404, detail="Practitioner profile not found")
    existing = await db.practitioner_verifications.find_one({"practitioner_id": prac["id"], "verification_type": body.verification_type})
    doc = {
...
@api.get("/me/practitioner/verifications")
...
async def decide_verification(verification_id: str, body: VerificationDecisionIn, user: dict = Depends(require_role("admin"))):
    ...
    if body.decision == "verified":
        # Grant requested badges
        for badge_type in body.grant_badges:
            existing = await db.practitioner_badges.find_one({"practitioner_id": v["practitioner_id"], "badge_type": badge_type})
            if existing:
                continue
        ...
        # Roll-up status: if practitioner has at least gov ID verified, mark verified
        gov = await db.practitioner_verifications.find_one({"practitioner_id": v["practitioner_id"], "verification_type": "government_id", "status": "verified"})
        if gov:
            await db.practitioners.update_one({"id": v["practitioner_id"]}, {"$set": {"verification_status": "verified"}})
        # If insurance badge granted, set is_insured=true
        if "insured" in body.grant_badges:
            await db.practitioners.update_one({"id": v["practitioner_id"]}, {"$set": {"is_insured": True}})
    return {"ok": True}
...
async def list_blog_posts(city: Optional[str] = None, category: Optional[str] = None):
    ...
    docs = clean_list(docs)
    # strip heavy body field from list
    for d in docs:
...
async def get_blog_post(slug: str):
    ...
    doc = clean(doc)
    # Hydrate embedded practitioner cards
    embed = []
